<template>
    <div class="cart">
        ...
        <div class="cart-empty-1" v-if="!cartList.length">购物车为空</div>
        <!--<div class="cart-empty" v-if="cartList.length" @click="handleCartEmpty">清空购物车</div>
-->
        <div class="cart-empty" v-if="cartList.length" @click="flag=true">清空购物车</div>
...
<script>
    ...
    export default {
        computed: {
            ...
            cartList () {//购物车中放的都是id和count
                return this.$store.state.cartList;
            },
            productDictList () {//真实的商品列表
                const dict = {};//将真实的列表数据放到dict中
        ...
        data () {
            return {
                //productList: product_data,//获取所有数据放到状态中，然后获取vuex中的id和count，通过id找到对应的商品，放到购物车列表
                // （更好）如果产品列表中的数据过多，就直接把需要加入购物车的商品所有信息都放到vuex中，，在vuex中获取数据放到购物车列表
                promotionCode: '',
                promotion: 0,
                flag:false
            }
